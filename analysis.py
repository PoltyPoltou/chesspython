...
class ThreadGameAnalysis(threading.Thread):
    def __init__(self, game: chess.Board, engineStr="./engines/stockfish12.exe", pv=3, threads=3, limit=chess.engine.Limit(depth=18)):
        super().__init__(self)
        self.engine: chess.engine.SimpleEngine = chess.engine.SimpleEngine.popen_uci(
            engineStr)
    ...
    def run(self):
        ...
        for move in self.moveStack:
            self.root.push(move)
            self.analyseList.append(self.engine.analyse(
                self.root, self.limit, multipv=self.pv))
